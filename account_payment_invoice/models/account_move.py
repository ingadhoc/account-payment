...
class AccountMove(models.Model):
    ...
    payment_token_id = fields.Many2one(
        "payment.token",
        string="automatic electonic payment",
        check_company=True,
        readonly=True,
    ...
    def action_post(self):
        ...
        to_pay_moves = self.filtered(
            lambda x: x.payment_token_id
            and x.state == "posted"
            and x.payment_state in ["not_paid", "electronic_pending"]
    ...
    def create_electronic_payment(self):
        ...
        for rec in self:
            active_transaction_amount = sum(
                rec.transaction_ids.filtered(lambda tx: tx.state in ["authorized", "done", "pending"]).mapped("amount")
            )
