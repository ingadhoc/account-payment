...
class AccountPaymentInvoiceWizard(models.TransientModel):
        ...
        if self.journal_id.l10n_latam_use_documents:
            ...
            if self._context.get("internal_type") == "debit_note":
                ...
                self.journal_document_type_id = (
                    document_types and document_types[0]._origin or refund.l10n_latam_document_type_id
                )
            else:
        ...
        for tax in self.tax_ids.filtered(lambda x: not x.price_include and x.amount_type not in ["fixed"] or x.amount):
            if tax.amount_type == "percent":
                ...
                tax_percent += tax.get_partner_alicuot(
                    self.payment_id.partner_id, fields.Date.context_today(self)
                ).alicuota_percepcion
            else:
                raise ValidationError(_("You can only set amount total if taxes are of type " "percentage"))
    ...
    def change_payment_group(self):
        ...
        tax_domain = [("type_tax_use", "=", type_tax_use), ("company_id", "=", self.payment_id.company_id.id)]
        ...
        self.amount_total = abs(self.payment_id.payment_difference)
        return {
            "domain": {
    ...
    def confirm(self):
        ...
        self.payment_id.to_pay_move_line_ids += invoice.open_move_line_ids
