...
class AccountPayment(models.Model):
    ...
    def _compute_to_pay_move_lines(self):
        ...
        for rec in with_payment_pro:
            if rec.partner_id != rec._origin.partner_id or rec.partner_type != rec._origin.partner_type or \
                ...
                rec.add_all(self.env.context.get('active_ids'))
    ...
    def add_all(self, active_ids=False):
        for rec in self:
            rec.to_pay_move_line_ids = [Command.clear(), Command.set(self.env['account.move.line'].search(rec.with_context(active_ids=active_ids)._get_to_pay_move_lines_domain()).ids)]
