...
class AccountPaymentMassTransfer(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if not checks[
            0
        ].l10n_latam_check_current_journal_id.inbound_payment_method_line_ids.filtered(
            lambda x: x.code == "in_third_party_checks"
        ):
            raise UserError(
                _(
                    "Checks must be on a third party checks journal to be transfered by this wizard"
                )
            )
