...
class AccountJournal(models.Model):
    ...
    l10n_latam_use_checkbooks = fields.Boolean(
        ...
        compute="_compute_l10n_latam_use_checkbooks",
        store=True,
        readonly=False,
    ...
    def _compute_l10n_latam_use_checkbooks(self):
        arg_checks = self.filtered(
            lambda x: not x.check_manual_sequencing
            and x.company_id.country_id.code
            in self._get_checkbooks_by_default_country_codes()
            and "check_printing" in x.outbound_payment_method_line_ids.mapped("code")
        )
