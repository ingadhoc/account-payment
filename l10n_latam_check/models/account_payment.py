...
class AccountPayment(models.Model):
    ...
    l10n_latam_check_warning_msg = fields.Html(
    ...
    l10n_latam_check_bank_id = fields.Many2one(
        ...
        readonly=True,
        states={"draft": [("readonly", False)]},
        compute="_compute_l10n_latam_check_data",
        store=True,
        string="Check Bank",
    )
    ...
    l10n_latam_checkbook_id = fields.Many2one(
        "l10n_latam.checkbook",
        "Checkbook",
        store=True,
        compute="_compute_l10n_latam_checkbook",
        readonly=True,
        states={"draft": [("readonly", False)]},
    )
    ...
    def _compute_l10n_latam_checkbook(self):
        with_checkbooks = self.filtered(
            ...
            and x.journal_id.l10n_latam_use_checkbooks
        )
        ...
        for rec in with_checkbooks:
            ...
            if (
                rec.l10n_latam_checkbook_id
                and rec.l10n_latam_checkbook_id in checkbooks
            ):
                continue
    ...
    def _compute_check_number(self):
        ...
        return super(
            AccountPayment,
            self.filtered(lambda x: not x.check_number) - from_checkbooks,
    ...
    def _inverse_check_number(self):
        ...
        avoid_inverse = self.filtered(
            lambda x: x.l10n_latam_checkbook_id
    ...
    def action_unmark_sent(self):
        ...
        if self.filtered("l10n_latam_checkbook_id"):
            raise UserError(
                _("Unmark sent is not implemented for checks that use checkbooks")
            )
        ...
        for rec in self.filtered(lambda x: x.state == "draft"):
            if rec.l10n_latam_check_id:
                ...
                if last_operation and last_operation[0].date > date:
                    ...
                    not rec.is_internal_transfer
                    and rec.payment_type == "inbound"
                    and rec.partner_type != last_operation.partner_type
                ):
                    rec.l10n_latam_check_warning_msg = _(
                        ...
                        "payment) so that the same receivable / payable account is used"
                    ) % (rec.partner_id.name)
                ...
                rec.l10n_latam_checkbook_id.range_to
                and rec.check_number.isdecimal()
                and int(rec.check_number) > rec.l10n_latam_checkbook_id.range_to
            ):
                rec.l10n_latam_check_warning_msg = _(
                    "The <strong>check number %s is bigger</strong> than max number for this checkbook.<br/>"
                    "Please check you're using the right check number and the right checkbook"
                ) % (rec.check_number)
    ...
    def action_post(self):
        ...
        for rec in self:
            if rec.l10n_latam_check_id and not rec.currency_id.is_zero(
                rec.l10n_latam_check_id.amount - rec.amount
            ):
                raise UserError(
                    _(
                ...
                if rec.l10n_latam_check_id.state != "posted":
                    ...
                    raise ValidationError(
                        _("Check '%s' is on journal '%s', we can't receive it again")
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None):
        ...
        check = (
            ...
            if (
                ...
                or self.l10n_latam_checkbook_id
            )
