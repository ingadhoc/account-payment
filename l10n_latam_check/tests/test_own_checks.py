...
class TestOwnChecks(L10nLatamCheckTest):
    def test_01_journals_creation(self):
        ...
        self.assertTrue(
            self.bank_journal.l10n_latam_use_checkbooks,
            "Bank journal don't have use checkbooks",
        )
    ...
    def test_01_pay_with_multiple_checks(self):
        ...
        vals_list = [
            {
                ...
                "payment_method_line_id": self.bank_journal._get_available_payment_method_lines(
                    "outbound"
                )
        ...
        for i, check_number in zip(range(3), [50, 120, 200]):
            ...
            self.assertTrue(
                payment.is_move_sent, "Check %s was not set sent" % payment.check_number
            )
            self.assertEqual(
                payment.l10n_latam_checkbook_id.next_number,
                check_number + 1,
                "Next sequence was not updated properly on checkbook %s"
