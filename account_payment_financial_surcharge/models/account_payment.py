...
class AccountPayment(models.Model):
    ...
    financing_surcharge = fields.Monetary(compute="_compute_financing_surcharge")
    available_card_ids = fields.Many2many(
        "account.card", string="Cards", related="payment_method_line_id.available_card_ids"
    )
    card_id = fields.Many2one(
        "account.card", string="Card", compute="_compute_financing_plan", store=True, readonly=False
    )
    ...
    net_amount = fields.Monetary(compute="_compute_net_amount", inverse="_inverse_net_amount")
