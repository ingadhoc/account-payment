...
class TestAccountPaymentFinancialSurcharge(common.TransactionCase):
    def setUp(self):
        ...
        self.first_company = self.env["res.company"].search(
            [("name", "=", "Muebleria ARG"), ("use_payment_pro", "=", True)]
        )
        ...
        installment_dic_1 = {"name": "Plan 1 cuota 10%", "installment": 1, "divisor": 1, "surcharge_coefficient": 1.01}
        installment_dic_2 = {"name": "Plan 3 cuotas 30%", "installment": 3, "divisor": 1, "surcharge_coefficient": 1.03}
        self.account_card = self.env["account.card"].create(
            {"name": "Visa1", "installment_ids": [(0, 0, installment_dic_1), (0, 0, installment_dic_2)]}
        )
        ...
        ).payment_term_surcharge_product_id = self.product_surcharge.id
    ...
    def test_payment_financial_surcharge_1(self):
        ...
        vals = {"journal_id": self.first_company_journal.id}
        ...
        self.assertEqual(
            round(payment.amount, 2), original_amount + payment.financing_surcharge, "Fallo el monto total"
        )
        self.assertEqual(
            payment.financing_surcharge,
            payment.matched_move_line_ids[1].balance,
