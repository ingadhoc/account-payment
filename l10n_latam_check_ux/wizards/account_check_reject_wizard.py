...
class AccountCheckRejectWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        for check in checks:
            if not check.can_reject:
                raise UserError(
                    _("Check '%s' cannot be rejected. Only third-party checks in valid states can be rejected.")
                    % check.name
                )
        return res
    ...
    def action_confirm(self):
        ...
        for check in checks:
            ...
            if (
                check.payment_method_code != "new_third_party_checks"
                or not check.can_reject
                or last_operation.journal_id == self.rejected_journal_id
            ):
                raise UserError(_("Check '%s' is not a third-party check and cannot be rejected.") % check.name)
            self._reject_check(check)

        # Redirect to check operations view
        return self._open_check_operations(checks)
    ...
    def _open_check_operations(self, checks):
        """Open the check operations view for the rejected checks."""
        # Collect all operations from all processed checks
        all_operations = self.env["account.payment"]
        for check in checks:
            operations = (check.operation_ids + check.payment_id).filtered(
        ...
        return {
            ...
            "domain": [("id", "in", all_operations.ids)],
        }
