...
class AccountCheckActionWizard(models.TransientModel):
    ...
    def action_confirm(self):
        ...
        check = self.env['l10n_latam.check'].browse(self._context.get('active_id', False))
        if self.date < check.payment_id.date:
            raise UserError(_('La fecha del débito del cheque %s no puede ser inferior a la fecha de emisión del mismo %s.') % (self.date, check.payment_id.date))
        ...
        wizard = self.env['account.reconcile.wizard'].with_context(active_model='account.move.line', active_ids=move_line_id).create(new_mv_line_dicts)
        wizard.reconcile()
        ...
        if debit_move:
            check.message_post(body=f'El cheque nro "{check.name}" ha sido debitado. ' + debit_move._get_html_link())
        else:
            check.message_post(body=f'El cheque nro "{check.name}" ha sido debitado, pero no se encontró el asiento asociado.')
    ...
    def _get_outstanding_account(self, check):
        ...
        if not journal_manual_payment_method:
            raise UserError(_("No es posible crear un nuevo débito de cheque sin un método de pagos 'manual' en el diario %s.") % (journal.display_name))
