...
class PaymentTransactionRetry(models.TransientModel):
    ...
    def default_get(self, default_fields):
        ...
        if active_model == 'account.move':
            move_ids = self.env[active_model].browse(active_ids).filtered(lambda x: x.payment_state in ['not_paid', 'partial'] and \
                x.amount_residual > 0.0 and x.move_type in ['out_invoice', 'out_refund'] and x.state == 'posted' and \
                    not {'pending','authorized'}.intersection(set(x.transaction_ids.mapped('state'))) and \
                    x.company_id == self.env.company)
            rec['line_ids'] = [Command.create({'invoice_id':x.id, 'amount_to_pay':x.amount_residual}) for x in move_ids]
...
class PaymentTransactionRetryLines(models.TransientModel):
    ...
    def _compute_payment_token_id(self):
        for rec in self:
            token_list = self.env['payment.token'].search([('company_id', '=', rec.retry_id.company_id.id), '|', ('partner_id', 'child_of', rec.commercial_partner_id.id), ('partner_id', '=', rec.commercial_partner_id.id)])
            if token_list:
