...
class AccountPaymentGroup(models.Model):
    ...
    def _compute_matched_move_line_ids(self):
        ...
        """
        for rec in self:
            payment_lines = rec.payment_ids.mapped("line_ids").filtered(
            ...
            debit_lines_sorted = debit_moves.filtered(
                lambda x: x.date_maturity != False
            ).sorted(key=lambda x: (x.date_maturity, x.move_id.name))
            credit_lines_sorted = credit_moves.filtered(
                lambda x: x.date_maturity != False
            ).sorted(key=lambda x: (x.date_maturity, x.move_id.name))
    ...
    def _compute_payments_amount(self):
        for rec in self:
            ...
            rec.payments_amount = sum(
                (
                    rec._origin.payment_ids
                    + rec.payment_ids.filtered(lambda x: not x.ids)
                ).mapped("l10n_ar_amount_company_currency_signed")
            )
    ...
    def _get_to_pay_move_lines_domain(self):
        ...
        return [
            ...
            (
                ...
                "=",
                "receivable" if self.partner_type == "customer" else "payable",
            ),
        ]
    ...
    def _unlink_if_not_posted(self):
        ...
        if recs:
            raise ValidationError(
                _("You can not delete posted payment groups. Payment group ids: %s")
                % recs.ids
            )
    ...
    def post(self):
        ...
        if posted_payment_groups:
            raise ValidationError(
                _(
                    "You can't post and already posted payment group. Payment group ids: %s"
                )
                % posted_payment_groups.ids
            )
        for rec in self:
            ...
            rec.payment_ids.filtered(
                lambda p: p.partner_id != rec.partner_id.commercial_partner_id
            ).write({"partner_id": rec.partner_id.commercial_partner_id.id})
            ...
            if not rec.receiptbook_id and not rec.name:
                rec.name = (
                    any(rec.payment_ids.mapped("name"))
                    and ", ".join(rec.payment_ids.mapped("name"))
                    or False
                )
    ...
    def check_to_pay_lines(self):
        for rec in self:
            ...
            if (
                to_pay_partners
                and to_pay_partners != rec.partner_id.commercial_partner_id
            ):
                raise ValidationError(
                    _(
                        "Payment group for partner %s but payment lines are of partner %s"
                    )
    ...
    def _inverse_document_number(self):
        for rec in self.filtered("document_type_id"):
            if not rec.document_number:
                ...
                rec.name = "%s %s" % (
                    rec.document_type_id.doc_code_prefix,
                    document_number,
                )
    ...
    def _compute_receiptbook(self):
        for rec in self.filtered(
            lambda x: not x.receiptbook_id
            or x.receiptbook_id.company_id != x.company_id
        ):
