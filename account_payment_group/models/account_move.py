...
class AccountMove(models.Model):
    ...
    open_move_line_ids = fields.One2many(
        "account.move.line", compute="_compute_open_move_lines"
    )
    ...
    def _check_unique_sequence_number(self):
        payment_group_moves = self.filtered(
            lambda x: x.journal_id.type in ["cash", "bank"]
            and x.payment_id.payment_group_id
        )
        return super(
            AccountMove, self - payment_group_moves
    ...
    def action_register_payment_group(self):
        ...
        return {
            ...
            "context": {
                ...
                if to_pay_move_lines[0].account_id.internal_type == "receivable"
                else "supplier",
    ...
    def pay_now(self):
        ...
        for rec in self:
            ...
            if (
                ...
                and rec.payment_state in ["not_paid", "patial"]
            ):
                ...
                payment_group = (
                    rec.env["account.payment.group"]
                    .with_context(pay_context)
                    .create(
                        {
                ...
                if (
                    partner_type == "supplier"
                    and payment_group.payment_difference >= 0.0
                    or partner_type == "customer"
                    and payment_group.payment_difference < 0.0
                ):
                    payment_type = "outbound"
                    payment_methods = (
                        pay_journal.outbound_payment_method_line_ids.payment_method_id
                    )
                else:
                    payment_type = "inbound"
                    payment_methods = (
                        pay_journal.inbound_payment_method_line_ids.payment_method_id
                    )
    ...
    def button_draft(self):
        self.filtered(lambda x: x.state == "posted" and x.pay_now_journal_id).write(
    ...
    def _get_last_sequence_domain(self, relaxed=False):
        ...
        if self.journal_id.type in ("cash", "bank") and not self.payment_group_id:
            ...
            where_string, param = super(
                AccountMove, self.with_context(without_payment_group=True)
