...
class AccountPayment(models.Model):
    ...
    label_journal_id = fields.Char(compute="_compute_label")

    label_destination_journal_id = fields.Char(compute="_compute_label")
    ...
    def _compute_available_journal_ids(self):
        ...
        for pay in self:
            filtered_domain = (
                ...
                else [("outbound_payment_method_line_ids", "!=", False)]
            )
            if pay.payment_group_id:
                filtered_domain.append(
                    ("company_id", "=", pay.payment_group_id.company_id.id)
                )
    ...
    def _compute_l10n_ar_amount_company_currency_signed(self):
        ...
        for payment in self:
            if (
                ...
                payment.l10n_ar_amount_company_currency_signed = (
    ...
    def create(self, vals_list):
        ...
        for rec in recs.filtered(
            lambda x: not x.payment_group_id and not x.is_internal_transfer
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None):
        ...
        if self.force_amount_company_currency:
            difference = (
                self.force_amount_company_currency - res[0]["credit"] - res[0]["debit"]
            )
            if res[0]["credit"]:
