...
class AccountPaymentGroupInvoiceWizard(models.TransientModel):
        ...
        if self.journal_id.l10n_latam_use_documents:
            ...
            if self._context.get("internal_type") == "debit_note":
                ...
                self.journal_document_type_id = (
                    document_types
                    and document_types[0]._origin
                    or refund.l10n_latam_document_type_id
                )
            else:
    ...
    def change_product(self):
        ...
        self.tax_ids = self.payment_group_id.partner_id.with_company(
            company
        ).property_account_position_id.map_tax(taxes)
        ...
        for tax in self.tax_ids.filtered(lambda x: not x.price_include):
            if tax.amount_type == "percent":
                ...
                tax_percent += tax.get_partner_alicuot(
                    self.payment_group_id.partner_id, fields.Date.context_today(self)
                ).alicuota_percepcion
            else:
