...
class AccountTax(models.Model):
    ...
    withholding_user_error_domain = fields.Char(
        default="[]", help="Write a domain over account voucher module"
    )
...
result = withholdable_base_amount * 0.10
    ...
    def check_withholding_non_taxable_amounts(self):
        for rec in self:
            if rec.withholding_non_taxable_amount > rec.withholding_non_taxable_minimum:
    ...
    def get_period_payments_domain(self, payment_group):
        ...
        previous_payments_domain = [
            ...
            ("date", "<=", to_date),
            ("date", ">=", from_date),
            ("payment_group_id.state", "not in", ["draft", "cancel", "confirmed"]),
            ("state", "!=", "cancel"),
    ...
    def get_withholding_vals(self, payment_group, force_withholding_amount_type=None):
        ...
        withholding_amount_type = (
            force_withholding_amount_type or self.withholding_amount_type
        )
        (
            withholdable_advanced_amount,
        ...
        if self.withholding_accumulated_payments:
            ...
            previous_withholding_amount = sum(
                ...
                .search(previos_payments_domain)
                .mapped("amount")
            )
        ...
        withholdable_base_amount = (
            (total_amount > withholding_non_taxable_minimum)
            and (total_amount - withholding_non_taxable_amount)
            or 0.0
        )
        ...
        if self.withholding_type == "code":
            ...
            safe_eval(
                self.withholding_python_compute, localdict, mode="exec", nocopy=True
            )
            ...
            period_withholding_amount = (
                (total_amount > withholding_non_taxable_minimum)
                and (withholdable_base_amount * percentage + fix_amount)
                or 0.0
            )
