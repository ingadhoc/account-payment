...
class AccountPayment(models.Model):
    ...
    def action_post(self):
        ...
        for payment in self.filtered("withholding_number"):
            payment.line_ids.filtered(
                lambda x: x.tax_line_id == payment.tax_withholding_id
                and (not x.name or x.name == "/")
            ).name = payment.withholding_number
    ...
    def _get_withholding_repartition_line(self):
        ...
        rep_line = self.tax_withholding_id[rep_field].filtered(
        ...
        if not rep_line.account_id:
            raise UserError(
                _("The tax %s dont have account configured on the tax repartition line")
