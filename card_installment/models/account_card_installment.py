...
class AccountCardInstallment(models.Model):
    ...
    name = fields.Char("Fantasy name", default="/")
    divisor = fields.Integer(
        string="Divisor",
    )
    installment = fields.Integer(
        string="installment plan", help="Number of installment"
    )
    ...
    bank_discount = fields.Float(string="bank discount", help="Bank discount")
    ...
    def map_installment_values(self, amount_total):
        ...
        return {
            ...
            "description": _(
                "%s installment of %.2f (total %.2f)"
                % (self.divisor, amount / self.divisor, amount)
            ),
        }
