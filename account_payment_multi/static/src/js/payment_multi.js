        ...
        if (items[i].checked) {
          ...
          if (old_group && group != old_group) {
            ...
            return new Dialog(null, {
              ...
              size: "medium",
              $content: _t(
                `<p>selected invoices must be in the same currency, partner and company</p>`
              ),
              buttons: [{text: _t("Ok"), close: true}],
            }).open();
          }
          total += parseFloat(items[i].dataset.amountResidual);
        }
      }
      this.el
        .querySelectorAll(".oe_amount")
        .forEach(
          (selector) => (selector.innerHTML = currency + " " + total.toFixed(2))
        );
      if (total) {
        this.el
          .querySelectorAll(".multi_payment_selector")
          .forEach((selector) => selector.classList.remove("invisible"));
      } else {
        this.el
          .querySelectorAll(".multi_payment_selector")
          .forEach((selector) => selector.classList.add("invisible"));
      }
    },
    _onPayNowBtnClick: function (event) {
      ...
      for (let i = 0; i < items.length; i++) {
        if (items[i].checked) {
          total += parseFloat(items[i].dataset.amountResidual);
          invoices.push({
            id: parseInt(items[i].dataset.invoiceId),
            token: items[i].dataset.accessToken,
          });
        }
      }
      const params = {invoice_ids: invoices, amount: total};
      return this._rpc({
