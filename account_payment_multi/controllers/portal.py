...
class PaymentPortal(portal.CustomerPortal):
    ...
    def invoice_multi_transaction(self, invoice_ids, amount, **kwargs):
        ...
        for invoice in invoice_ids:
            ...
            invoices_sudo += self._document_check_access(
                "account.move", invoice["id"], invoice.get("token")
            )
        payment_link = (
            request.env["payment.link.wizard"]
            .sudo()
            .with_context(
                active_id=invoices_sudo[0].id,
                active_ids=invoices_sudo.ids,
        ...
        if (
            float_compare(
                payment_link.amount_max,
                amount,
                precision_rounding=payment_link.currency_id.rounding or 0.01,
            )
            == -1
        ):
    ...
    def payment_pay(
        ...
        **kwargs
        ...
        if "invoice_ids" in kwargs:
            invoice_ids = [
                int(x) for x in kwargs["invoice_ids"].split(",") if x.isdigit()
            ]
            invoices_sudo = (
    ...
    def _create_transaction(
        self, *args, invoice_ids=None, custom_create_values=None, **kwargs
    ):
