...
class AccountPayment(models.Model):
    ...
    installment_id = fields.Many2one(
        "account.card.installment", string="Installment plan"
    )
    tiket_number = fields.Char("Tiket number")
    lot_number = fields.Char("Lot number")
    net_amount = fields.Monetary(
        compute="_computed_net_amount", inverse="_inverse_net_amount"
    )

    @api.depends("available_card_ids", "payment_type")
    @api.onchange("payment_method_line_id")
    def _compute_financing_plan(self):
        with_plan = self.filtered(
            lambda x: x.payment_type == "inbound" and x._origin.available_card_ids
        )
    ...
    def _inverse_net_amount(self):
        for rec in self:
            ...
            ).amount = rec.net_amount * (rec.installment_id.surcharge_coefficient or 1)
