...
class AccountPaymentGroup(models.Model):
    ...
    def _computed_financing_surcharge(self):
        for rec in self:
            rec.financing_surcharge = sum(
                rec.payment_ids.filtered("installment_id").mapped(
                    lambda x: x.amount - x.net_amount
                )
            )
    ...
    def post(self):
        ...
        if self.filtered(lambda p: p.financing_surcharge > 0):
            ...
            wiz = (
                self.env["account.payment.group.invoice.wizard"]
                .with_context(
                    is_automatic_subcharge=True,
                    active_id=self.id,
            ...
            if self.env.context.get("open_invoice_payment"):
                ...
                draft_invoices = move_lines and move_lines.mapped("move_id").filtered(
                    lambda x: x.state == "draft"
                )
                if draft_invoices:
                    # remove draft invoice from debt
                    self.to_pay_move_line_ids -= self.to_pay_move_line_ids.filtered(
