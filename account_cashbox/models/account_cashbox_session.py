...
class AccountCashboxSession(models.Model):
    ...
    state = fields.Selection(
        POP_SESSION_STATE, required=True, readonly=False, tracking=True, index=True, copy=False, default="draft"
    )
    ...
    _sql_constraints = [
        ("uniq_name_cashbox", "unique(name, cashbox_id)", "El nombre de esta sesión de caja debe ser único!")
    ]
            ...
            if not rec.allow_concurrent_sessions:
                ...
                for journal in rec.cashbox_id.cash_control_journal_ids:
                    leaf = [
                        ...
                        ("cashbox_session_id.state", "=", "closed"),
                    ]
                    last_session = (
                        rec.env["account.cashbox.session"]
                        .sudo()
                        .search(
                            [("cashbox_id", "=", rec.cashbox_id.id), ("state", "=", "closed")],
                    ...
                    balance_start[journal.id] = (
                        rec.env["account.cashbox.session.line"].sudo().search(leaf, limit=1).balance_end_real
                    )
    ...
    def _check_active_cashbox(self):
        for rec in self.filtered(lambda x: x.state != "closed" and not x.cashbox_id.allow_concurrent_sessions):
            other_opened_sessions = self.search(
                [("state", "!=", "closed"), ("id", "!=", rec.id), ("cashbox_id", "=", rec.cashbox_id.id)]
            )
