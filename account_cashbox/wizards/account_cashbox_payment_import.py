...
class AccountCashboxPaymentImport(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if "cashbox_session_id" in fields:
            pop_session = (
                self.env["account.cashbox.session"].browse(self.env.context["active_id"])
                if self.env.context.get("active_model") == "account.cashbox.session"
                else self.env["account.cashbox.session"]
            )
